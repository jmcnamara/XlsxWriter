...
#. Check the code names that macros use to refer to the workbook and
   worksheets. In general VBA uses a code name of ``ThisWorkbook`` to refer to
   the current workbook and the sheet name (such as ``Sheet1``) to refer to the
   worksheets. These are the defaults used by XlsxWriter. If the macro uses
   other names then you can specify these using the workbook and worksheet
   :func:`set_vba_name` methods::
